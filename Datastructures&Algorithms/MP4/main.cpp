...
int main()
{
    ...
    while (!passFile.eof() && !passwords.isFull())
    {
        ...
        passFile >> tempPass.username; //get username and add to temppass
        passFile >> tempPass.password;
        passwords.insert(tempPass); // insert temppass object to hash table
    }

    cout << "Printing the hash table:..." << endl;
    //**add line here to show (print) the HashTbl
    
    passwords.showStructure();  //show the hashtable structure
    ...
    while (cin >> name)  // to quit, type CTRL Z in Visual C++
    {
        ...
        userFound = passwords.retrieve(name, tempPass); //check if the username and password are in passwords
        if(userFound == true)
        {
        ...
        //**or "Authentication successful"
            ...
            else {cout << "Authentication failure.\n\n";}
        }
        else {cout << "Authentication failure, username not found.\n" << endl;}
    }
    cout << endl;
}
